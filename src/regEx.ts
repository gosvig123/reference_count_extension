...
function tokenizeJS(code: string): string[] {
  // Tokenize the code, splitting on whitespace and preserving important symbols
  return code.split(/(\s+|\(|\)|=|=>|{|}|,|\.|new)/).filter((token) => token.trim() !== "");
}
...
function findJSFunctionUsages(tokens: string[]): string[] {
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (
      ...
      (i === 0 || tokens[i - 1] !== "function") &&
      (i < 2 || tokens[i - 2] !== "function")
    ) {
      usages.push(tokens[i]);
    } else if (i + 2 < tokens.length && tokens[i + 1] === "=" && tokens[i + 2] === ">") {
      // This catches arrow function definitions
      continue;
    } else if (i > 0 && tokens[i - 1] === "." && i + 1 < tokens.length && tokens[i + 1] === "(") {
      // This catches method invocations like 'instance.classMethod()'
      usages.push(tokens[i]);
    }
  }
  return usages;
...
function findReactFunctionDefinitions(tokens: string[]): string[] {
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (
      ...
      tokens[i] === "React" &&
      i + 2 < tokens.length &&
      ["memo", "forwardRef"].includes(tokens[i + 2])
    ) {
      functions.push(tokens[i - 1]);
    }
  }
